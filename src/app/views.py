...
def add_pokemon():
    ...
    for pokemon_data in pokemon_data_list:
        ...
        existing_values = (
            db.session.query(Pokemon).filter_by(name=pokemon_data["name"]).first()
        )
    ...
    try:
        ...
        db.session.rollback()
        return {"success": False, "message": "Failed to add/update Pokemon"}, 500
...
def get_legendary_pokemons():
    try:
        ...
        return jsonify({"success": True, "legendary_pokemons": pokemon_list}), 200
